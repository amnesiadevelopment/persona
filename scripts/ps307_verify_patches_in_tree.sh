...
for patch_path in "${PATCHES[@]}"; do
  ...
  LC_ALL=C awk -v MAX_PER_FILE="$MAX_PER_FILE" -f "$EVIDENCE_AWK" "$patch_path" "$patch_path" > "$ev"
  ...
  if [ "$MODE" = "present" ]; then
    ...
    if [ "$((adverse * 2))" -gt "$claims" ]; then patch_failed=yes; else patch_failed=no; fi
  fi
  ...
  if [ "$patch_failed" = "no" ]; then
    ...
    if [ "$MODE" = "present" ]; then
      ...
      printf '%-45s ❌ PRESENT IN THE CONTROL (%d of %d claims hold — a majority, so this is corroborated and not a coincidence)%s\n' "$patch_name" "$adverse" "$claims" "$detail" >> "$REPORT"
      echo "::error::PS-307: ${patch_name} appears to be PRESENT in the ${TREE} tree, which must carry none of our patches — ${adverse} of ${claims} evidence claims hold."
    fi
  fi
done
