...
UNCOVERED_SURFACES: tuple[tuple[str, str], ...] = (
    ...
    (
        "every check on this list runs on FIREFOX only — WITH TWO EXCEPTIONS",
        "most of the scratch profiles here take Context.make_profile's "
        ...
        "fixtures, not of the instrument. THE EXCEPTIONS are the two SURVIVOR "
        "checks — no-process-survives-a-closed-session (PS-347) and "
        "no-process-survives-a-degraded-session (PS-388) — which launch "
        "os_type=linux and therefore CHROMIUM on purpose: they count surviving "
        "processes, and the leak they guard was measured on the WRAPPER, "
        "multi-process launch. They observe ONE engine on ONE platform "
        "(chromium/Linux). Every other check below still says nothing about "
        "how a macos, linux or mobile profile behaves. Widening the fixtures "
        "is separate work.",
    ),
    (
        "NO SURVIVOR CHECK OBSERVES THE FIREFOX ENGINE TREE — and the reason "
        "is the INSTRUMENT, not firefox's shape",
        "⛔ THE REASON THIS ENTRY USED TO GIVE WAS FALSE AND IS CORRECTED "
        "RATHER THAN DELETED (PS-402), because a plausible wrong reason stops a "
        "reader looking for the real one. It said 'a direct single-process "
        "launch does not leak on terminate() at all, so the same measurement "
        "taken on the firefox fixtures would be vacuous'. BOTH HALVES WERE "
        "WRONG. A persona firefox launch is a FORK launch (_spawn_invisible -> "
        "InvisibleProcess) above a 6-to-12 process Gecko tree — a forkserver, "
        "a Socket Process, WebExtensions, an RDD Process, a Utility Process and "
        "up to four Web Content children, the count driven by how many tabs are "
        "open — measured three times by three tickets, and the three figures "
        "are quoted rather than averaged because they disagree by design: "
        "readings/ps171-2026-08-25/REPRO.md:407 (6 processes at one tab, 11 at "
        "two), readings/ps349-2026-09-09/jugwedge.txt (nproc 10 across 300 "
        "samples, 12 across 105), and readings/ps402-2026-09-10/ (11 members, "
        "named one by one, on the shipped Personium build under the survivor "
        "check's own sampler). A measurement there would not be vacuous; it "
        "would be the arm PS-8 DoD #1 asks for. ⭐ THE ACTUAL OBSTACLE, "
        "measured on 8 consecutive launches: the ENGINE calls setsid, so the "
        "Gecko leader's pgid == sid == its own pid — a DIFFERENT group from the "
        "one record_group_by_construction stored, which is the group the FORKED "
        "PYTHON LEADER leads. process_group_survivors(recorded_group(proc)) "
        "therefore counts TWO processes (that leader plus the playwright node "
        "driver) while the browser is eleven, and the two populations do not "
        "intersect at all. ⛔ SO _MIN_LIVE_TREE (3) IS UNSATISFIABLE: peak 3, "
        "oscillating 2<->3, never settling inside the 90s _TREE_GROW_TIMEOUT — "
        "a CEILING, not a slow curve, so a wider timeout cannot reach it. And "
        "the falsification cannot be satisfied either (run_check falsifies "
        "FIRST): the pre-PS-192 sabotage leaves ZERO survivors in the group AND "
        "zero in the engine session, twice. A firefox arm copied from the "
        "chromium ones would report CANNOT_RUN (exit 2) on every run forever, "
        "so it was deliberately NOT added — a check that can never certify "
        "anything is worse than a recorded gap. ⭐ THE CURE, so the next slice "
        "is specified: anchor a survivor count on the ENGINE'S OWN SESSION "
        "(os.getsid of the engine leader) instead of on the launch group. That "
        "is a different INSTRUMENT, not a second profile. ⚠️ AND THIS IS NOT A "
        "LEAK REPORT. Firefox's teardown is CORRECT on every path the product "
        "takes — terminate_process_group sends SIGTERM to the group first, "
        "_child's SIGTERM handler runs session.teardown(), and the tree goes: 0 "
        "of 10 survivors both on a healthy session (terminate() in 1.62s) and "
        "on one whose whole engine tree was SIGSTOPped (5.43s / 6.05s). What is "
        "unobserved is that correctness, which is what a gate is for. ⚠️ ONE "
        "BOUND WORTH KEEPING: under a SIGKILL-FIRST teardown the wedged tree "
        "leaks entirely (10 of 10 survive, twice) because the in-child handler "
        "cannot run and the group signal never reached the engine. NOTHING IN "
        "THE PRODUCT TAKES THAT PATH, so it is not a shipped defect — it is the "
        "failure mode a session-anchored gate would catch and a group-anchored "
        "one structurally cannot.",
    ),
    (
        "the firefox in_process (THREAD) arm's teardown",
        "InvisibleProcess.terminate() only sets a stop event on that arm "
        "(invisible_launch.py's else branch), so a session that ignores it "
        "leaves a live browser thread behind — baseline._teardown records this. "
        "Unreachable from this lane by construction rather than by choice: "
        "self._fork = needs_fork_launch() and not in_process, and "
        "needs_fork_launch() is IS_LINUX, so the thread arm is the "
        "Windows/macOS path and no parameter value reaches it here. A separate "
        "check on a separate venue, and NOT the same gap as the entry above — "
        "that one is about an instrument that cannot see a tree, this one is "
        "about a teardown that does not signal one.",
    ),
    (
    ...
    (
        ...
        "SELECTED check has needs_launch=True (6 of the 9: restart-continuity, "
        "two-profile-unlinkability, benign-edit-stability, "
        "trash-restore-and-wipe, no-process-survives-a-closed-session, "
        "no-process-survives-a-degraded-session), and "
        ...
        "those 6 launching checks use firefox fixtures (above), which really "
        "do launch, and the other two are CHROMIUM launches that need a "
        "display just as much — they start a real headful browser rather than "
        ...
        "message instead of six identical ones. The consequence is stated "
