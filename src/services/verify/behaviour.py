...
UNCOVERED_SURFACES: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "processes, and the leak they guard is a property of the WRAPPER, "
        ...
        "fixtures would be vacuous. They observe ONE engine on ONE platform "
        "(chromium/Linux) and say nothing about the other arm — and the "
        "firefox arm is not merely unobserved but KNOWN to differ: on the "
        "in_process path terminate() only sets a stop event, so a session that "
        "ignores it leaves a live browser thread behind (baseline._teardown "
        "records this). That arm is a separate check that does not exist yet. "
        "Every other "
