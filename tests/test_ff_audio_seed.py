...
def test_firefox_cloak_uses_spidermonkeys_native_form(tmp_path):
    """V8's one-line ``function x() { [native code] }`` is WRONG on Firefox.

    SpiderMonkey renders three lines with a four-space indent. Emitting V8's
    form here would leave a detector one comparison away from the masking tell
    the cloak exists to remove — the same reasoning as
    invisible_launch._native_cloak_js, and pinned there too.
    """
    js = firefox_audio_init_script(1)
    assert "String.fromCharCode(10)" in js
    assert "    [native code]" in js
    assert "{ [native code] }" not in js, (
        "V8's one-line native form is emitted on the Firefox path, which is "
        "itself a masking tell on SpiderMonkey"
    )


def test_firefox_cloak_adds_no_own_property_to_wrappers(tmp_path):
    """The Firefox cloak must not use native_ext's ``__pnaName`` marker.

    Nothing on the Firefox path reads that marker (no persona extension is
    loaded), so it would not be a cloak there — just an own property on every
    wrapper that a page can enumerate. Live-verified on a launched profile:
    getOwnPropertyNames(getChannelData) == [length, name, prototype].
    """
    assert "__pnaName" not in _FIREFOX_NATIVE_WRAP, (
        "the Firefox cloak carries native_ext's marker, which nothing reads on "
        "that engine — an enumerable tell rather than a hiding place"
    )
    assert "WeakMap" in _FIREFOX_NATIVE_WRAP


def test_firefox_cloak_chains_rather_than_replaces_tostring(tmp_path):
    """Two other init scripts already patch Function.prototype.toString in this
    realm (the locale and outer-size overrides). A flag-guarded patch would let
    only the first win; chaining composes."""
    js = firefox_audio_init_script(1)
    assert "__pts.apply(this, arguments)" in js, (
        "the Firefox cloak does not delegate to the previously-installed "
        "toString, so it cannot compose with the locale/outer-size cloaks"
    )
